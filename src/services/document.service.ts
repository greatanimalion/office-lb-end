...
export const createDocument = async (
  ...
  db.run(
    `INSERT INTO documents (title, filename, filepath, owner_id) VALUES ("${title}", "${filename}", "${filepath}", ${ownerId})`
  )
...
export const trackDocumentUpdate = async (id: number): Promise<void> => {
  const db = getDB()
